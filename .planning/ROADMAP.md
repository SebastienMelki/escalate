...
### Phase 12: Fix Session Summary, Snooze, and Message Context

**Goal**: Session summary sends before hook exits, Snooze button appears in escalation messages, file paths display in Slack context, and PreToolUse timeout has safety buffer
**Depends on**: Phase 11
**Requirements**: INTL-05, SLCK-02, SLCK-03
**Gap Closure:** Closes 3 requirement gaps + 1 flow gap from v1.0 third audit
**Success Criteria** (what must be TRUE):

1. `requestSummary()` is awaited (or given time to flush) before `process.exit(0)` in on-task-completed.ts and on-stop.ts — session summary actually sends
