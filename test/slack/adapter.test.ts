...
describe('SlackAdapter', () => {
  ...
  describe('sendEscalation', () => {
    it('posts message and returns escalation ID', async () => {
      ...
      // Returns a valid UUID (generated by randomUUID fallback)
      expect(escalationId).toMatch(
        /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/,
      );
      expect(escalationId).not.toBe('undefined');
    ...
    it('uses provided id instead of generating a new UUID', async () => {
      const adapter = createAdapter();
      const request = makeRequest({ id: 'pre-assigned-uuid-1234' });

      const escalationId = await adapter.sendEscalation(request);

      expect(escalationId).toBe('pre-assigned-uuid-1234');
      expect(mockPostMessage).toHaveBeenCalledOnce();
    });
  ...
  describe('end-to-end action resolution', () => {
    it('round-trips caller-provided UUID through Block Kit action_id to store.resolve', async () => {
      // Create a store record first (simulating what http-bridge does)
      ...
      const adapter = createAdapter();
      const escalationId = await adapter.sendEscalation(makeRequest({ id: record.id }));

      // The adapter used the caller-provided ID
      expect(escalationId).toBe(record.id);

      // Extract the posted Block Kit blocks
      const postCall = mockPostMessage.mock.calls[0] as [{ blocks: Array<{ type: string; elements?: Array<{ action_id?: string }> }> }];
      const blocks = postCall[0].blocks;
      const actionsBlock = blocks.find((b) => b.type === 'actions');
      expect(actionsBlock).toBeDefined();

      // Extract the first button's action_id
      const elements = actionsBlock?.elements ?? [];
      const firstElement = elements[0];
      expect(firstElement).toBeDefined();
      const actionId = firstElement?.action_id ?? '';
      expect(actionId).toContain(record.id);
      ...
      await (actionHandler as NonNullable<typeof actionHandler>).handler({
        ack: vi.fn(),
        action: { action_id: actionId, type: 'button', text: { type: 'plain_text', text: 'Allow' } },
        body: { user: { id: 'U123' }, message: { ts: 'mock.ts.1234' }, channel: { id: 'C12345' } },
        client: { chat: { update: vi.fn() } },
      });

      // Verify the store record is resolved using the same ID
      const resolved = store.getById(record.id);
